...
class CreateDataSet:
    ...
    def get_movieid_movie_map(self):
        movieid_movie_map = {}
        movieid_movie_query = 'select movieid, moviename from .moviedata.mlmovies'
        # Execute Query
        movieid_movie_list = self.get_data_from_kdb(movieid_movie_query)
        for elem in movieid_movie_list:
            movieid_movie_map[elem[0]] = np.array(elem[1]).tolist()
        return movieid_movie_map

    def get_tagid_tag_map(self):
        movieid_movie_map = {}
        movieid_movie_query = 'select tagId, tag from .moviedata.genome_tags'
        # Execute Query
        movieid_movie_list = self.get_data_from_kdb(movieid_movie_query)
        for elem in movieid_movie_list:
            movieid_movie_map[elem[0]] = np.array(elem[1]).tolist()
    ...
    def get_movies_by_user(self,input_user):
        movie_user_query = 'exec movieid from  (`userid xgroup .moviedata.mlratings) uj `userid xgroup .moviedata.mltags where userid=`$"' + input_user + '"'
        # Execute Query
        movie_list = self.get_data_from_kdb(movie_user_query)
        movies_user = np.array(movie_list).tolist()
        return movies_user[0]
