...
class TfIdf:
    ...
    def get_movie_tags_space(self):
        ...
        movie_tag_query = "select movieid, tagid, timestamp from (0!`movieid xgroup .moviedata.mlratings) ij `movieid xgroup .moviedata.mltags"
        ...
        for elem in movie_tag_data:
            ...
            if(elem[0] not in movie_tag_map.keys()):
                ...
                #movie_rank_map[elem[0]] = len(array) * np.array(elem[3]).tolist()
            else:
                movie_tag_map[elem[0]] += np.array(elem[1]).tolist()
                movie_timestamp_map[elem[0]] += array
                #movie_rank_map[elem[0]] += len(array) * np.array(elem[3]).tolist()
